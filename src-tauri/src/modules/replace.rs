    ...
    async move {
        ...
        for rule in &rules {
            let matcher = match rule.mode {
                0 => {
                    let original = rule.original_id.as_ref().ok_or(anyhow::anyhow!("unknow original"))?;
                    let replacement = rule.replacement_id.as_ref().ok_or(anyhow::anyhow!("unknow replacement"))?;
                    RuleMatcher::IdMatch {
                ...
                1 => {
                    let original = rule.original_details.as_ref().ok_or(anyhow::anyhow!("unknow original"))?;
                    let replacement = rule.replacement_details.as_ref().ok_or(anyhow::anyhow!("unknow replacement"))?;
            ...
            for (matcher, global) in &rule_cache {
                    ...
                    RuleMatcher::FullMatch { original, .. } => {
                        block.block.id == original.id &&
                            ...
                            block.block.properties.iter().all(|(k, v)| {
                                original.properties.get(k.as_ref())
                ...
                if matches {
                    let new_block = match matcher {
                        RuleMatcher::IdMatch { replacement, .. } => {
                            BlockData {
                                id: BlockId {
                        ...
                        RuleMatcher::FullMatch { replacement, .. } => {
                            replacement.clone()
                        }
                    };
